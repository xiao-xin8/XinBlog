...
async function login(request, env) {
  ...
  const accessToken = await signJWT(
    { sub: user.id, username: user.username, role: user.role, type: 'access', iat: nowSec, exp: nowSec + 3600 },
    env.JWT_SECRET
  );
...
async function updateAdminComment(request, env, user) {
  ...
  await env.DB_POSTS.prepare(`UPDATE comments SET ${updates.join(', ')} WHERE id = ?`).bind(...params).run();
  return jsonResponse(0, null, '保存成功');
}
