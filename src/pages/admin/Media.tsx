...
import { fetchAdminMedia, fetchAdminMediaDetail, updateAdminMedia, fetchAdminMediaUsage, fetchAdminMediaUsageDetail, type AdminMediaUsageDetail } from '@/api/admin';
...
export function AdminMedia() {
  ...
  const [usageDetail, setUsageDetail] = useState<AdminMediaUsageDetail | null>(null);
  ...
  const loadUsageDetail = async () => {
    ...
    const res = await fetchAdminMediaUsageDetail();
    if (res) setUsageDetail(res);
    setUsageDetailLoading(false);
  };

  const openUsageDetail = () => {
    setUsageDetailOpen(true);
    if (!usageDetail) loadUsageDetail();
  };

  const closeUsageDetail = () => {
    setUsageDetailOpen(false);
  };
  ...
  const renderStorageUsage = () => {
    ...
    return (
      <Tooltip title={usage ? `点击查看详情 · 共 ${usage.count} 个文件` : '点击获取存储统计'}>
        <Paper
          elevation={0}
          onClick={() => !usageLoading && openUsageDetail()}
          sx={{
            display: 'flex',
            alignItems: 'center',
            gap: { xs: 1, sm: 1.5 },
            ...
            borderRadius: 5,
            flexShrink: 0,
            cursor: usageLoading ? 'default' : 'pointer',
        ...
        >
          {usageLoading ? (
            ...
            <CloudQueue sx={{ fontSize: 18, color: `${color}.main` }} />
          )}
          <Box sx={{ width: { xs: 48, sm: 96 } }}>
            <LinearProgress
              variant="determinate"
              value={percent}
              sx={{
          ...
          <Typography
            variant="caption"
            fontWeight={600}
            sx={{ whiteSpace: 'nowrap', color: `${color}.dark`, fontSize: { xs: '0.7rem', sm: '0.75rem' } }}
          >
  ...
  return (
    <Fade in timeout={400}>
      <Box>
        ...
        >
          <DialogTitle sx={{ fontWeight: 700 }}>存储空间详情</DialogTitle>
          <DialogContent>
            {usageDetailLoading && !usageDetail ? (
              ...
              <Stack spacing={2}>
                <Box>
                  <Typography variant="h4" fontWeight={700} color="primary.main">
                    {formatBytes(usageDetail.totalSize)}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    精确统计 · 已用 {Math.min(100, Math.round((usageDetail.totalSize / (500 * 1024 * 1024)) * 100))}%
                  </Typography>
                </Box>
                <LinearProgress
                  variant="determinate"
                  value={Math.min(100, (usageDetail.totalSize / (500 * 1024 * 1024)) * 100)}
                  sx={{
                    height: 8,
                    borderRadius: 4,
                    bgcolor: 'action.hover',
                ...
                <Grid container spacing={1.5}>
                  {[
                    { label: '原始文件大小', value: formatBytes(usageDetail.rawSize) },
                    { label: 'base64 数据', value: formatBytes(usageDetail.base64Size) },
                    { label: '分片数据', value: formatBytes(usageDetail.chunkSize) },
                    { label: '文件数量', value: `${usageDetail.count} 个` },
                    { label: '膨胀比例', value: `${usageDetail.ratio}x` },
                    { label: '免费额度', value: formatBytes(500 * 1024 * 1024) },
                  ].map((item) => (
              ...
              <Typography color="text.secondary">点击重新计算获取精确用量</Typography>
            )}
          </DialogContent>
          <DialogActions>
            <Button onClick={loadUsageDetail} disabled={usageDetailLoading} startIcon={usageDetailLoading ? <CircularProgress size={16} /> : <Refresh />}>
              重新计算
            </Button>
            <Button onClick={closeUsageDetail} variant="contained">关闭</Button>
          </DialogActions>
        </Dialog>
