...
export function AdminPosts() {
  ...
  const editorPanel = (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, minWidth: 0, gap: 2, position: 'relative', overflow: 'hidden' }}>
      <Box sx={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', height: '100%', gap: 2, overflow: 'auto', overscrollBehavior: 'contain' }}>
      {editorLoading && (
        <Box
          sx={{
            position: 'absolute',
      ...
      >
        ...
        >
          ...
          <Dialog open={inlineImageDialogOpen} onClose={() => setInlineImageDialogOpen(false)} fullWidth maxWidth="xs" TransitionComponent={Grow} PaperProps={{ sx: { borderRadius: { xs: 2, sm: '12px' } } }} BackdropProps={{ 'aria-hidden': false }}>
            ...
            <DialogContent>
              <Stack spacing={2}>
                <Button variant="outlined" component="label" startIcon={<ImageIcon />} fullWidth sx={{ textTransform: 'none', borderRadius: 2, py: 1 }}>
                  上传本地图片
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Box sx={{ display: 'flex', gap: 1.5, width: '100%', flexDirection: { xs: 'column-reverse', sm: 'row' }, justifyContent: { sm: 'flex-end' }, minWidth: 0 }}>
            ...
            <Button color="error" variant="contained" onClick={handleConfirmRemoveCover} fullWidth={isMobileAdmin} sx={{ textTransform: 'none', borderRadius: 2 }}>
              确认移除
            </Button>
          </Box>
        </DialogActions>
