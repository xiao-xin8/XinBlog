...
export function useLive2dEditor() {
  ...
  const [width, setWidth] = useState<number | string>(current.width);
  const [height, setHeight] = useState<number | string>(current.height);
  const [mobileWidth, setMobileWidth] = useState<number | string>(current.mobileWidth ?? DEFAULT_LIVE2D_CONFIG.mobileWidth ?? 180);
  const [mobileHeight, setMobileHeight] = useState<number | string>(current.mobileHeight ?? DEFAULT_LIVE2D_CONFIG.mobileHeight ?? 180);
  ...
  useEffect(() => {
    ...
    setMobileWidth(cfg.mobileWidth ?? DEFAULT_LIVE2D_CONFIG.mobileWidth ?? 180);
    setMobileHeight(cfg.mobileHeight ?? DEFAULT_LIVE2D_CONFIG.mobileHeight ?? 180);
    setTools(cfg.tools);
    setDrag(cfg.drag);
  ...
  const isDirty = useMemo(() => {
    ...
    if (Number(width) !== cfg.width) return true;
    if (Number(height) !== cfg.height) return true;
    if (Number(mobileWidth) !== (cfg.mobileWidth ?? DEFAULT_LIVE2D_CONFIG.mobileWidth ?? 180)) return true;
    if (Number(mobileHeight) !== (cfg.mobileHeight ?? DEFAULT_LIVE2D_CONFIG.mobileHeight ?? 180)) return true;
  ...
  const buildConfig = (): Live2dConfig => ({
    ...
    mobileWidth: mobileEnabled ? Number(mobileWidth) : undefined,
    mobileHeight: mobileEnabled ? Number(mobileHeight) : undefined,
    tools,
    drag,
    showToggleAfterQuit,
    logLevel,
  ...
  const resetToDefault = () => {
    ...
    setWidth(cfg.width);
    setHeight(cfg.height);
    setMobileWidth(cfg.mobileWidth ?? 180);
    setMobileHeight(cfg.mobileHeight ?? 180);
    setTools(cfg.tools);
    setDrag(cfg.drag);
    setShowToggleAfterQuit(cfg.showToggleAfterQuit);
    setLogLevel(cfg.logLevel);
  ...
  const validateDimensions = (label: string, value: number | string): boolean => {
    ...
    if (Number.isNaN(num) || num < 100) {
      enqueueSnackbar(`${label}必须大于或等于 100 像素`, { variant: 'error' });
      return false;
    }
    return true;
  };
