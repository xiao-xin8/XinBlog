...
function getCacheTtl(path: string): number {
  ...
  if (cleanPath.includes('/comments') || cleanPath.includes('/likes')) return 0;
...
async function refreshAccessToken(): Promise<boolean> {
  ...
  try {
    ...
    const data = (await res.json()) as { code: number; data?: { accessToken: string; refreshToken: string; user: unknown }; msg?: string };
